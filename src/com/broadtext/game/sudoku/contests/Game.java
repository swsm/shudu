...
public class Game {
    ...
    private Point[] tempPoint; //存储问题占位符位置
    ...
    public String getAnswers() {
        out(result);
        int length;
        length = tempPoint.length;
        String answer;
        answer = new String();
        String str = ChangeArrayToString(result);
        System.out.println("*******************************");
        System.out.println(str);
        for (int i = 0; i < length; i++) {
            answer += String.valueOf(str.charAt(tempPoint[i].x * 9 + tempPoint[i].y));
        }
     ...
     * @return 转换后的数组
     */
    public int[][] ChangeStringToArray(String question) {
        String str;
        str = new String();
        str = question.replaceAll("[?]", "0");
        int[][] array = new int[9][9];
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                array[i][j] = Integer.valueOf(String.valueOf(str.charAt(i * 9 + j)));
            }
        }
        out(array);
        return array;
