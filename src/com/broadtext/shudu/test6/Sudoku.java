package com.broadtext.shudu.test6;
...
public class Sudoku extends JFrame {
    ...
    Object level = JOptionPane.showInputDialog(null, "1.初级--2.低级--3.中级--4高级--5.骨灰级", "请选择难度",
            JOptionPane.QUESTION_MESSAGE, null, fivelevel, fivelevel[2]);
                            ...
                            for (int i = 0; i < 9; i++) {
                                for (int j = 0; j < 9; j++) {
                                    if (button[i][j].getBackground() == Color.WHITE
                                            || button[i][j].getBackground() == Color.RED) { //将玩家已经输入值的按钮设置成不可操作
        ...
        hideanswer.addActionListener(new ActionListener() { //隐藏答案的监听事件
                    ...
                    public void actionPerformed(ActionEvent e) {
                        for (int i = 0; i < 9; i++) {
                            for (int j = 0; j < 9; j++) {
                                if (button[i][j].getBackground() == Color.WHITE
                                        || button[i][j].getBackground() == Color.RED) { //将玩家已经输入值的按钮设置成可操作
